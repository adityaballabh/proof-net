...
void sendWrapper(char *buf, int sockfd, uint16_t len) {
  ...
  if (bytes_left)
    throw runtime_error("send incomplete: " + to_string(bytes_left) +
                        " bytes pending");
}
...
Receipt parseReceipt(string receipt_str) {
  ...
  string pref;
  Receipt receipt;
  ss >> pref >> receipt.packet_id >> receipt.generator >> receipt.receiver >>
      receipt.bytes >> receipt.signature;
  return receipt;
}

string getReceiptPayload(Receipt receipt) {
  return receipt.packet_id + ' ' + to_string(receipt.generator) + ' ' +
         to_string(receipt.receiver) + ' ' + to_string(receipt.bytes);
}
...
string getBase64Encoded(unsigned char *data, int len) {
  ...
  return sodium_bin2base64(b64_encoded, b64_len, data, len,
...
string getBase64Decoded(string encoded) {
  ...
  size_t bin_len;
  if (sodium_base642bin((unsigned char *)bin.data(), bin.size(),
                        encoded.c_str(), encoded.size(), NULL, &bin_len, NULL,
                        sodium_base64_VARIANT_URLSAFE)) {
...
bool isValidReceipt(Receipt receipt, PubKey pub_key) {
  ...
  if (sodium_base642bin(signature, sizeof(signature), receipt.signature.c_str(),
                        receipt.signature.size(), NULL, NULL, NULL,
                        sodium_base64_VARIANT_URLSAFE))
    return false;

  bool valid =
      !crypto_sign_verify_detached(signature, (unsigned char *)payload.c_str(),
                                   payload.size(), pub_key.signing);
  return valid;
}
...
void recvWrapper(char *buf, int sockfd, uint16_t len) {
  ...
  if (bytes_left)
    throw runtime_error("recv incomplete: " + to_string(bytes_left) +
                        " bytes pending");
}
...
unordered_map<int, PubKey> getPubKeys(unordered_map<int, Node> &nw_config,
...
int getNodeID(unordered_map<int, Node> &nw_config, map<int, Node> &acct_config,
...
string getEncrypted(unsigned char *pub_key, string message) {
  ...
  crypto_box_seal((unsigned char *)encrypted.data(),
                  (unsigned char *)message.data(), message.size(), pub_key);
  return encrypted;
}

string getOnionEncrypted(unordered_map<int, PubKey> &pub_keys, deque<int> route,
                         vector<string> salts, vector<string> signatures,
                         string packet_id, string content) {
...
Layer getOnionDecrypted(PubKey &node_pub, unsigned char *pvt_encryption,
                        string encoded, bool skip_headers) {
  ...
  if (crypto_box_seal_open((unsigned char *)decoded.data(),
                           (unsigned char *)bin.data(), bin_len,
                           node_pub.encryption, pvt_encryption)) {
    layer.next_hop = INT_MIN;
    return layer;
  }
  ...
  int id_start = HOP_ID_LEN, salt_start = id_start + PACKET_ID_B64_LEN,
      signature_start = salt_start + SALT_LEN,
...
void signReceipt(Receipt &receipt, unsigned char *pvt_key) {
  ...
  crypto_sign_detached(signature, NULL, (unsigned char *)payload.c_str(),
                       payload.size(), pvt_key);
...
void writeNodeState(NodeState node_state, int node) {
  ...
  out << node_state.allowed << ' ' << node_state.forwarded << ' '
      << node_state.used << ' ';
...
string getHash(string salt, int hop) {
  ...
  crypto_hash_sha256((unsigned char *)hash_out.data(),
                     (unsigned char *)hash_in.data(), hash_in.size());
  return hash_out;
}

void processPacket(unordered_map<int, Node> &nw_config,
                   unordered_map<int, PubKey> &pub_keys, Packet packet,
  ...
  if (prev_node != -1 && nw_config.count(prev_node)) {
    ...
    string encrypted_receipt =
        getOnionEncrypted(pub_keys, {prev_node}, {}, {}, "",
  ...
  string msg = packet_id + getHash(layer.salt, node_id),
         payload = layer.payload;
  auto acct_pub_key = pub_keys[ACCT_COMMON_ID];
  bool is_valid = !crypto_sign_verify_detached(
      (unsigned char *)layer.signature.data(), (unsigned char *)msg.data(),
      msg.size(), acct_pub_key.signing);

  if (!is_valid) {
    cout << "\nsignature verification failed for packet " + packet_id
         << ". dropping packet\n";
    return;
  }
  ...
  string b64_payload =
      getBase64Encoded((unsigned char *)payload.data(), payload.size());
...
bool canSend(Proof proof, unordered_map<int, PubKey> &pub_keys,
             string packet_id, int node) {
  NodeState node_state = getNodeState(node);
  for (Receipt r : proof.receipts) {
    if (node_state.receipt_ids.count(r.packet_id) ||
        !isValidReceipt(r, pub_keys[r.generator]) || r.generator == node)
      continue;
  ...
  int max_used = node_state.used + MAX_LEN,
      thresh = node_state.forwarded * 2 + INIT_ALLOWED;
  cout << "\n"
       << "node" << node << " used: " << max_used
       << " forwarded: " << node_state.forwarded << " threshold: " << thresh;
  if (max_used <= thresh) {
...
void handleProof(map<int, Node> &acct_config,
                 unordered_map<int, PubKey> &pub_keys,
                 unsigned char *pvt_signing, unsigned char *pvt_encryption,
                 string packet_str, int new_fd, int acct_id, int sender_id) {
  if (!isCorrectAcct(acct_config, acct_id, sender_id)) {
    ...
    string encrypted_resp =
        getOnionEncrypted(pub_keys, {sender_id}, {}, {}, "", acct_resp);
    sendPacket(encrypted_resp, new_fd);
    return;
  }

  stringstream ss(packet_str);
  string pref, encrypted_proof_str, receipts_str, commitments_str, packet_id,
      commitment;
  ...
  try {
    Layer layer = getOnionDecrypted(pub_keys[acct_id], pvt_encryption,
                                    encrypted_proof_str, true);
    if (layer.next_hop == INT_MIN)
      throw runtime_error("proof decryption failed");
  ...
  if (canSend(proof, pub_keys, packet_id, sender_id)) {
    for (string commitment : commitments) {
      ...
      if (decoded_commitment.empty())
        ...
        unsigned char signature[crypto_sign_BYTES];
        crypto_sign_detached(signature, NULL, (unsigned char *)msg.data(),
                             msg.size(), pvt_signing);
...
void handleBootstrapReq(unordered_map<int, Node> &nw_config,
                        unordered_map<int, vector<int>> &adj, int sockfd) {
  ostringstream out;
...
void fetchTopology(unordered_map<int, Node> &nw_config,
                   unordered_map<int, vector<int>> &adj, Node acct,
                   int node_id) {
...
void processConnections(unordered_map<int, Node> &nw_config,
                        map<int, Node> &acct_config,
                        unordered_map<int, PubKey> &pub_keys,
                        unordered_map<int, vector<int>> &adj,
                        unsigned char *pvt_signing,
                        unsigned char *pvt_encryption, int sockfd, int node_id,
                        HostType host_type) {
  struct sigaction sa;
  ...
  while (true) {
    ...
    if (!fork()) {
      ...
      } else if (packet_str.substr(0, PROOF_PREFIX.size()) == PROOF_PREFIX &&
                 host_type == HostType::Acct)
        handleProof(acct_config, pub_keys, pvt_signing, pvt_encryption,
                    packet_str, new_fd, node_id, prev_node);
      else if (packet_str.substr(0, BOOTSTRAP_REQ_PREFIX.size()) ==
                   BOOTSTRAP_REQ_PREFIX &&
               host_type == HostType::Acct)
...
bool routeExistsLen(unordered_map<int, vector<int>> &adj,
                    unordered_set<int> &vis, deque<int> &route, int node,
...
deque<int> computeRoute(unordered_map<int, vector<int>> &adj, int src,
                        int dest) {
  int min_len = getMinDist(adj, src, dest);
  if (min_len == -1)
    throw runtime_error("cannot reach " + to_string(dest) + " from " +
                        to_string(src));
  deque<int> route;
  unordered_set<int> vis;
...
void init(unordered_map<int, Node> &nw_config, map<int, Node> &acct_config,
          unordered_map<int, PubKey> &pub_keys,
          unordered_map<int, vector<int>> &adj, unsigned char *pvt_signing,
          unsigned char *pvt_encryption, HostType host_type,
          string nw_config_path, string acct_config_path, int node_id,
          int argc) {
  srand(time(0));
  if (sodium_init() == -1)
    throw runtime_error("sodium_init failed");
  if (argc != 2) {
    string err_str = string("usage: ") +
                     (host_type == HostType::Node ? "node" : "acct") + " <id>";
