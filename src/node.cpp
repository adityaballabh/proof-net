...
vector<pair<Message, Packet>> loadMessages(unordered_map<int, vector<int>> &adj,
                                           vector<int> &delays, int node_id) {
  ...
  while (getline(in, line)) {
    ...
    cout << "\nscheduled message. prev packet id: " << message.id
         << ", new packet id: " << msg_id << '\n';
    message.id = msg_id;
    ...
    for (int hop : message.route) {
      ...
      string enc_hash = getBase64Encoded((unsigned char *)hash_out.data(),
...
void validateAndSendPacket(unordered_map<int, Node> &nw_config,
  ...
  if (canSendPacket(acct_config, pub_keys, proof, packet, pvt_encryption,
    ...
    packet.payload =
        getOnionEncrypted(pub_keys, message.route, packet.salts,
                          packet.signatures, message.id, message.content);
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    init(nw_config, acct_config, pub_keys, adj, pvt_signing, pvt_encryption,
         host_type, "", BOOTSTRAP_CONFIG_PATH, node_id, argc);
    ...
    if (!fork()) {
      ...
      vector<int> delays;
      vector<pair<Message, Packet>> msg_pkt_pairs =
          loadMessages(adj, delays, node_id);
      ...
      for (int i = 0; i < packet_cnt; i++) {
        ...
        validateAndSendPacket(nw_config, acct_config, pub_keys, proof, message,
                              packet, pvt_signing, pvt_encryption, node_id);
      }
      exit(0);
    }
